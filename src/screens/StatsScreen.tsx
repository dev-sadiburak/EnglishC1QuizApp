...
export default function StatsScreen() {
  ...
  const DistributionBar = () => (
    <View style={styles.distContainer}>
        <Text style={styles.distTitle}>Veri Dağılımı ({totalCount} Kelime)</Text>
        ...
        <View style={styles.distLegend}>
            <Text style={styles.legendText}><View style={[styles.dot, {backgroundColor:'#e74c3c'}]} /> Kritik (%{distribution.red.toFixed(0)})</Text>
            <Text style={styles.legendText}><View style={[styles.dot, {backgroundColor:'#f1c40f'}]} /> Gelişiyor (%{distribution.yellow.toFixed(0)})</Text>
            <Text style={styles.legendText}><View style={[styles.dot, {backgroundColor:'#27ae60'}]} /> Mastered (%{distribution.green.toFixed(0)})</Text>
        </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Veri Analizi</Text>
        <View style={styles.badge}>
             <Text style={styles.badgeText}>Genel Doğruluk: %{generalAccuracy}</Text>
        </View>
      </View>
        ...
        ListEmptyComponent={
            <View style={styles.emptyState}>
                <Text style={styles.emptyText}>
                    {activeTab === 'weak' ? "Harika! Kritik kelimen yok." : "Henüz ustalaştığın kelime yok."}
                </Text>
            </View>
        }
        renderItem={({ item }) => (
          <View style={styles.wordRow}>
            <View style={styles.wordInfo}>
              ...
              <Text style={styles.statDetail}>
                  {item.correct} Doğru / {item.total} Toplam
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  legendText: { fontSize: 11, color: '#555', alignItems: 'center', display: 'flex' },
  dot: { width: 8, height: 8, borderRadius: 4, marginRight: 4, display: 'inline-block' }, // Inline style fix for View inside Text issue logic
